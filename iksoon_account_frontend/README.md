...
- 📊 **실시간 통계**: 월별 수입, 지출, 잔액 현황
  - 💰 **다양한 지출 분석 옵션**: 상단 탭으로 차트 유형 선택 (기본: 카테고리별 지출)
  - **카테고리별 지출**: 지출 카테고리별 통계 및 차트
  - **고정/변동 지출 분석**: 지출 유형별 상세 분석
    - **📊 비교 차트**: 가로 막대 그래프로 고정 vs 변동 지출 한눈에 비교
    - 금액과 비율을 시각화하여 지출 패턴 파악
    - 요약 카드로 고정/변동 지출 상세 정보 제공
    - 탭으로 고정/변동 전환하여 카테고리별 상세 보기
  - **💳 결제수단별 지출**: 어떤 결제수단을 가장 많이 사용했는지 분석
    - 결제수단별 금액 및 비율 도넛 차트
    - 결제수단별 순위 리스트
    - 정렬 기능으로 높은순/낮은순 전환
    - 결제수단 클릭 시 해당 결제수단으로 결제한 실제 지출 내역 테이블 표시
      - 날짜, 카테고리, 키워드, 메모, 사용자, 금액 정보
      - 기간별 필터링 자동 적용
  - **👤 사용자별 지출**: 사용자별 지출 패턴 분석
    - 사용자별 금액 및 비율 도넛 차트
    - 사용자별 순위 리스트
    - 정렬 기능으로 높은순/낮은순 전환
    - 사용자 클릭 시 해당 사용자의 실제 지출 내역 테이블 표시
      - 날짜, 카테고리, 키워드, 결제수단, 메모, 금액 정보
      - 기간별 필터링 자동 적용
  - 카테고리 및 차트 클릭 시 키워드별 상세 분석 제공
...
- 🔍 **스마트 키워드 검색**: 키워드 기반 거래 내역 검색
  - **주 선택**: 달력에서 특정 주를 선택하여 해당 주의 데이터 조회
  - **월 선택**: 연도와 월을 편리하게 선택하여 특정 월 데이터 조회
  - **연도 선택**: 연도를 스크롤하여 선택하고 전체 연도 데이터 조회
  - **기간 지정**: 사용자 정의 시작일/종료일로 원하는 기간 검색
