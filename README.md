...
- **기준치 설정**: 카테고리별 월/연 기준치 설정 및 관리
- **실시간 사용량 추적**: 지출 기록 시 기준치 대비 사용량 실시간 계산
- **지능형 알림**: 기준치 초과 시 자동 알림 및 권장사항 제공
- **시각적 모니터링**: 진행률 바와 색상으로 직관적인 사용량 표시
- **통계 연동**: 통계 대시보드에서 기준치 정보 종합 분석
...
- `GET /category-budgets?user={user}&category_id={id}` - 기준치 목록 조회
- `POST /category-budgets/create` - 기준치 생성
- `PUT /category-budgets/update?id={id}` - 기준치 수정
- `DELETE /category-budgets/delete?id={id}` - 기준치 삭제
- `GET /category-budgets/usage?user={user}&category_id={id}` - 기준치 사용량 조회
...
- `GET /v2/out-account?date={YYYY-MM-DD}` - 지출 내역 조회
- `POST /v2/out-account/insert` - 지출 내역 생성
- `POST /v2/out-account/insert-with-budget` - 지출 내역 생성 (기준치 정보 포함)
- `PUT /v2/out-account/update` - 지출 내역 수정
- `DELETE /v2/out-account/delete?uuid={uuid}` - 지출 내역 삭제

- `GET /v2/in-account?date={YYYY-MM-DD}` - 수입 내역 조회
- `POST /v2/in-account/insert` - 수입 내역 생성
- `PUT /v2/in-account/update` - 수입 내역 수정
- `DELETE /v2/in-account/delete?uuid={uuid}` - 수입 내역 삭제

### 통계

- `GET /statistics?type={type}&category={category}&user={user}` - 통계 조회 (기준치 정보 포함)
...
- **기준치 관리**: 카테고리별 월/연 기준치 설정 및 사용량 모니터링
...
- `category_budgets`: 카테고리별 기준치 정보 (월/연 기준치)
- `keywords`: 키워드 정보
