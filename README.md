...
- ✅ 통계 및 리포트
  ...
  - **💰 고정/변동 지출 분석**: 지출 유형별 상세 분석 (선택적 표시)
    - 상단 탭으로 "카테고리별 지출", "고정/변동 지출 분석", "결제수단별 지출", "사용자별 지출" 선택
    - **📊 고정 vs 변동 지출 비교**: 가로 막대 그래프로 한눈에 비교
      - 고정/변동 금액 및 비율 시각화
      - 요약 카드로 상세 정보 제공
    - 고정 지출: 월세, 보험료, 구독료 등 정기적 지출 카테고리 통계
    - 변동 지출: 식비, 쇼핑, 교통비 등 변동적 지출 카테고리 통계
    - **💳 결제수단별 지출 분석**: 어떤 결제수단을 가장 많이 사용했는지 시각화
      - 결제수단별 금액 및 비율 도넛 차트
      - 결제수단별 순위 리스트
      - 결제수단 클릭 시 해당 결제수단으로 결제한 실제 지출 내역 테이블 표시 (날짜, 카테고리, 키워드, 메모, 사용자, 금액)
    - **👤 사용자별 지출 분석**: 사용자별 지출 패턴 시각화
      - 사용자별 금액 및 비율 도넛 차트
      - 사용자별 순위 리스트
      - 사용자 클릭 시 해당 사용자의 실제 지출 내역 테이블 표시 (날짜, 카테고리, 키워드, 결제수단, 메모, 금액)
    - 카테고리 및 차트 클릭 시 키워드별 상세 조회 가능
- ✅ 결제수단 관리
- ✅ 입금경로 관리
